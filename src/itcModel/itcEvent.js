...
itcEvent.Instance = function Instance(eventValue) {
    ...
    this.normalize = function normalize() {
        if (!that.isLeaf && that.left.isLeaf && that.right.isLeaf && that.left.value == that.right.value) {
    ...
    this.isEquivalent = function isEquivalent(anotherEvent) {
        ...
        return (
            !that.isLeaf &&
            !anotherEvent.isLeaf &&
            that.value == anotherEvent.value &&
            that.left.isEquivalent(anotherEvent.left) &&
            that.right.isEquivalent(anotherEvent.right)
        );
    };
    this.leq = function leq(anotherEvent) {
        ...
        if (!that.isLeaf) {
            ...
            if (!anotherEvent.isLeaf) {
                return (
                    cloneAndLiftClone(that.value, that.left).leq(
                        cloneAndLiftClone(anotherEvent.value, anotherEvent.left)
                    ) &&
                    cloneAndLiftClone(that.value, that.right).leq(
                        cloneAndLiftClone(anotherEvent.value, anotherEvent.right)
                    )
                );
            }
            ...
            // to param leaf
            return (
                cloneAndLiftClone(that.value, that.left).leq(anotherEvent) &&
