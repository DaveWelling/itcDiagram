...
itcId.Instance = function Instance(startValue) {
    ...
    this.split = function split() {
        var result = [new itcId.Instance(0), new itcId.Instance(0)];
        result[0].setAsNode();
        result[0].value = 0;
        result[1].setAsNode();
        result[1].value = 0;
...
itcId.sum = function sum(id0, id1) {
    ...
    if (id0.isLeaf && id0.value == 0) {
        ...
        throw new Error(
            'Sum operation on IDs failed.  ' + "First Id's value: " + id0.value + ".  Second Id's value: " + id1.value
        );
    }
    return newId;
};
